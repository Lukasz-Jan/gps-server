...
import com.gps.devices.transmit.*;
...
class GpsDataSenderTest extends TestBase {
    ...
    void send_empty_position_no_registration(int positionsQuantity) {
        ...
        await()
                ...
                .until(
                        new AsyncFetcher<>(dbClient, FetchMode.DEVICES, positionsQuantity),
                        equalTo(0L)
                );
    }
    ...
    @ParameterizedTest
    @ValueSource(ints = {1})
    void send_registration_with_wrong_body(int registrationsQuantity) {

        Assertions.assertDoesNotThrow(() -> gpsDataSender.sendRegistrationWithWrongBody(registrationsQuantity));
        await()
                .atMost(Duration.ofSeconds(5))
                .with()
                .pollDelay(Duration.ofSeconds(4))
                .until(
                        new AsyncFetcher<>(dbClient, FetchMode.DEVICES, registrationsQuantity),
                        equalTo(0L)
                );
    }
    ...
    @ParameterizedTest
    @ValueSource(ints = {1})
    void having_address_null_send_registration(int registrationsQuantity) {

        Assertions.assertDoesNotThrow(() -> gpsDataSender
                .sendRegistrationWithNullAddress(registrationsQuantity));
    ...
    private void generate_registrations_positions_then_send(int devicesQuantity, int positionsPerDevice,
        int commitRegisterationsTime, int registrationsPollInterval) throws NoMacAddress {

        gpsDataSender.sendRegisterAndItsPositions(devicesQuantity, positionsPerDevice,
                commitRegisterationsTime, registrationsPollInterval);
    }
}