...
public class RegisterRequestCreator {
    ...
    private final RegistrationMessageFactory registrationMessageFactory = new RegistrationMessageFactory();

    public List<RegistrationRequestDataWrapper> createRegisterRequests(int noOfDevices) throws URISyntaxException {

        List<RegistrationRequestDataWrapper> registerRequests = new ArrayList<>();

        for (int i = 0; i < noOfDevices; i++) {
            RegisterMessage registerMessage = registrationMessageFactory.createRandomRegisterMessage();
            ...
            HttpRequest registerRequest =
                    HttpRequest
                            ...
                            .build();
            registerRequests.add(new RegistrationRequestDataWrapper(registerMessage.macAddress(), registerRequest));
        }
        return registerRequests;
    }

    public List<RegistrationRequestDataWrapper> createRegisterRequests(int noOfDevices, String registerEndPoint) throws URISyntaxException {
        ...
        for (int i = 0; i < noOfDevices; i++) {
            ...
            HttpRequest registerRequest =
                    HttpRequest
                            ...
                            .header("Content-Type", "application/json")
                            .header("Accept", "application/json")
                            .timeout(Duration.ofSeconds(Constants.REGISTER_TIMEOUT))
                            .POST(bodyPublisherString)
                            .build();
            registerRequests.add(new RegistrationRequestDataWrapper(registerMessage.macAddress(), registerRequest));
        }
    ...
    public HttpRequest createRegisterRequest(String address) throws URISyntaxException {
        ...
        RegisterMessage registerMessage = registrationMessageFactory.createRandomRegisterMessage(address);
        String registerJson = msgUtils.serializeRegister(registerMessage);
